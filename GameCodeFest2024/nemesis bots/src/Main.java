...
public class Main {
    ...
    private static final String PLAYER_NAME = "test-Su";
        ...
        Emitter.Listener onMapUpdate = new Emitter.Listener() {
            ...
            public void call(Object... args) {
                try {
                    ...
                    while ((!currentEnemy.getIsAlive() || !PathUtils.checkInsideSafeArea(currentEnemy, gameMap.getDarkAreaSize() + 1, gameMap.getMapSize())
                    ...
                    switch (choice){
                        case 1:{
                            ...
                            if (!checkMoved) {
                                if (currentGun==null && currentMelee.getId().compareTo("HAND")!=0 && currentThrowable != null){
                                    System.out.println("Find gun");
                                    final List<Weapon> gunList = SubClass.sortWeapon(gameMap.getAllGun(), me);
                                    int m = 0;
                                    Weapon targetGun = gunList.get(m);
                                    while (m<gunList.size()-1 && !PathUtils.checkInsideSafeArea(targetGun, gameMap.getDarkAreaSize() + 4, gameMap.getMapSize())) {
                                        m++;
                                        targetGun = gunList.get(m);
                                    }
                                    if (SubClass.equal(me, targetGun)) {
                                        hero.pickupItem();
                                    } else {
                                        avoidNodeList.addAll(otherPlayers);
                                        String path = PathUtils.getShortestPath(gameMap, avoidNodeList, me, targetWeapon, false);
                                        hero.move(path); checkMoved = true;
                                    }

                                }
                                else if (targetWeapon!=null && targetChest!=null && SubClass.calDistance(me, targetWeapon) <= SubClass.calDistance(me, targetChest)) {
                        ...
                        if (currentHealingItems.isEmpty()==false && SubClass.calDistance(me, currentEnemy) > 8 && me.getHp()<=90){
